...
@Component({
  selector: 'howell-garbage-management-chart-record-event-mixed-into',
  imports: [CommonModule, GarbageManagementChartRecordEventComponent],
  templateUrl:
    './garbage-management-chart-record-event-mixed-into.component.html',
  styleUrl: './garbage-management-chart-record-event-mixed-into.component.less',
  providers: [GarbageManagementChartRecordEventBusiness],
})
export class GarbageManagementChartRecordEventMixedIntoComponent {
  @Input() date = new Date();
  constructor(private business: GarbageManagementChartRecordEventBusiness) {}
  private type = EventType.MixedInto;
  ...
  private load(date: Date) {
    this.business.load(this.type, date).then((x) => {
      this.datas = x
